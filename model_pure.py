...
class EncoderProcessDecoder(torch.nn.Module):
    ...
    def forward(self, goal, loop, y, free, collided, edge_index, k=10, **kwargs):

        goal = goal.view(-1, self.config_size)

        node_code = self.node_code(torch.cat((y, goal.repeat(len(y), 1), (y-goal)**2, y-goal), dim=-1))
        edge_code = self.edge_code(torch.cat((y[edge_index[0, :]], y[edge_index[1, :]]), dim=-1))

        # free_code = self.free_code(free).max(dim=0)[0]
        # collided_code = self.collided_code(collided).max(dim=0)[0]
        #
        # env_code = self.env_code(torch.cat((obs_code, free_code, collided_code), dim=-1))
        node_free_code = self.node_free_code(y)
        edge_free_code = self.edge_free_code(torch.cat((y[edge_index[0, :]], y[edge_index[1, :]]), dim=-1))

        goal_index = knn(y, goal, k=1)[1]
        ...
        policy_output = policy.new_zeros(len(y), len(y))
        policy_output[edge_index[1, :], edge_index[0, :]] = policy.squeeze()
        return value, policy_output, self.node_free(node_free_code), self.edge_free(edge_free_code)
