...
class BITStar:
    def __init__(self, environment, starts, maxIter=5, plot_flag=False, batch_size=200, T=1000):
        self.env = environment

        goal, bounds = tuple(environment.goal_state), environment.bound
        ...
        self.c_min = min([self.distance(start, self.goal) for start in starts])
    ...
    def get_f_score(self, point):
        # f^(x)
        return self.dist_to_src(point) + self.heuristic_cost(point, self.goal)

    def dist_to_src(self, point):
        src_dist = min([self.heuristic_cost(start, point) for start in self.starts])
        return src_dist
    ...
    def expand_vertex(self, point):
        ...
        for neighbor in neigbors_sample:
            estimated_f_score = self.dist_to_src(point) + \
    ...
    def get_best_path(self):
        ...
        if self.g_scores[self.goal] != INF:
            ...
            point = self.goal
            while point not in self.starts:
    ...
    def plan(self, pathLengthLimit, refine_time_budget=None, time_budget=None):
        ...
        while self.T < self.T_max and (time() - init_time < time_budget):
            ...
            if best_edge_value < self.g_scores[self.goal]:
                ...
                actual_f_edge = self.dist_to_src(bestEdge[0]) + actual_cost_of_edge + self.heuristic_cost(bestEdge[1], self.goal)
                if actual_f_edge < self.g_scores[self.goal]:
                    ...
                    if actual_g_score_of_point < self.get_g_score(bestEdge[1]):
                        ...
                        if bestEdge[1] == self.goal:
                            self.start = self.get_best_path()[0]
