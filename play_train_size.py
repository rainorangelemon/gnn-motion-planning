...
a = pickle.load(open("data/train_size.p", "rb"))
b = pickle.load(open("data/train_size_2.p", "rb"))
...
method_names = [0.2, 0.5, 0.7, 1.0, 2.0]
...
for metric_id, metric_title in enumerate(zip(metrics, titles)):
    ...
    for env, env_dimension in zip(envs, envs_dimension):
        for method in method_names:
            if metric == 'collision check':
                data[env].append(rd[env, method][metric_id] / rd[env, 2.0][metric_id])
    ...
    fig = go.Figure(
        [go.Scatter(name=env_dimension, x=[m*2000 for m in method_names], y=data[env], text=data[env], marker_color=color) for color, env, env_dimension in zip(colors, envs, envs_dimension)]
    )
    # Change the bar mode
    ...
    if "collision" in metric:
        fig.update_layout(yaxis_range=[0.9, 1.3])
    if 'path cost' in metric:
        fig.update_layout(yaxis_range=[0.9, 1.25])
    fig.update_xaxes(tick0=500, dtick=500)
